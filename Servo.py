...
def checkSum(buf):
    ...
    sum = ~sum  #取反
    return 0xff & sum  #取低8位

def servoCmd(id, cmd, par1 = None, par2 = None):
    """生成控制舵机的命令
    ...
    try:
        ...
        par_3=bytearray([(0xff & id), (0xff & cmdlen), (0xff & cmd)])
        buf.extend(par_3)
...
async def readCmd(id,cmd,par1=None,par2=None,distance=1,timeout=5):
    """发出读指令，有数据返回
    ...
    while True:
        ...
        if(timeout<=0):
            break
        retdata=await md.__uartSend(cmd,md.__calculate_time(distance))
        if retdata!=None:
            flag,_result=__rev(retdata) #判断报文是否正确
        else:
            flag=-1
            _result="报文为空"
        if flag==0:
    ...
    lds.loginfo("send_cmd",4,"cmd:"+str(cmd)+"...接收报文错误："+str(ret_data))
    return flag,ret_data #发送失败，传回错误信息

    
async def writeCmd(id,cmd,par1=None,par2=None,distance=1,timeout=5):
    """发出写指令，无数据返回
    ...
    while True:
        ...
        try:
            ...
            lds.loginfo("__uartSend",4,"cmd:"+str(cmd)+'--发送指令失败：'+ str(e))
        finally:
            await asyncio.sleep(md.__calculate_time(distance))
...
async def scan():
    try:
        ...
        await writeCmd(2,1,0,1000*60)  #转到0°，花费60s时间
        await asyncio.sleep(60)
    except Exception as e:
